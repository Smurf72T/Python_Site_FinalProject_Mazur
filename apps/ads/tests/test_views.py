...
from apps.ads.models import Category, Ad, Review, RentalRequest
...
class RentalRequestViewsTest(TestCase):
    ...
    def test_create_rental_request_own_ad(self):
        ...
        response = self.client.post(reverse('ads:create_rental_request', kwargs={'pk': self.ad.pk}), {
    ...
    def test_respond_to_request_owner(self):
        ...
        response = self.client.post(reverse('ads:respond_to_request', kwargs={'pk': request.pk}), {
            'action': 'accept'
        })
        self.assertEqual(response.status_code, 302)
